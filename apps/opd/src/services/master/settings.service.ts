...
export const settingsService = {
  async upsertSettings(input: CreateOrUpdateSettings): Promise<OpdSettings> {
  ...
  ): Promise<OpdSettings | null> {
    logger.info("entering::getSettings::service");

    const isCacheable = await checkIsCacheable(SHORT_CODE.SETTINGS);
    let settings: OpdSettings | null = null;
    if (isCacheable) {
      const cached = (await getAllCache(cacheKey)) as OpdSettings[];
