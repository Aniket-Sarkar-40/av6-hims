...
import { axiosClient, interceptor } from "@/config/axiosClient";
...
export const authService = {
  async loginExternal(payload: LoginPayload): Promise<ApiLoginResponse> {
    ...
    const { data } = await axiosClient.post<ApiLoginResponse>(EXT_LOGIN_URL!, payload);
    logger.info("exiting::loginExternal::service");
    return { token: data.token, shortToken: data.shortToken, uuid: data.uuid };
  },
  ...
  async login(credentials: LoginPayload): Promise<LoginResponse> {
    ...
    try {
      ...
      if (!token || !shortToken || !uuid) throw new ErrorHandler(401, "Invalid credentials: No token received");
  ...
  async userDetails(): Promise<UserResponse> {
    ...
    if (!user || !permission || !token) {
      throw new ErrorHandler(401, "User not authenticated or permissions not found");
    }
    ...
    const res: UserResponse = {
      user: data.data,
      permissions: Array.isArray(permission) ? permission : Array.from(permission ?? []),
      roles: user?.roles || [],
      currentRole: { id: currentRoleId as string, name: currentRoleName as string },
    };
    logger.info("exiting::userDetails::service");
    return res;
  },

  async changeRoleEXT(role: string, ccId: string, shortToken: string): Promise<ApiLoginResponse> {
    logger.info("entering::changeRoleEXT::service");
    const client = interceptor(shortToken);
    const { data, status } = await client.post<ChangeRoleExtWire>(EXT_CHANGE_ROLE_URL!, {
      roleId: role,
      ccId: ccId,
    });
  ...
  async changeRole(role: string, ccId: string): Promise<LoginResponse> {
    ...
    try {
      ...
      const { token, shortToken, uuid } = await this.changeRoleEXT(role, ccId, prevToken);
      logger.info("exiting::changeRole::service");
      if (!token || !shortToken || !uuid) throw new ErrorHandler(404, "You are not authorized to change this role");
      return { token, shortToken, uuid };
    } catch (error) {
      if (error instanceof AxiosError && error.response) {
        const message =
  ...
  async getRoleByCcId(ccId: number) {
    ...
    const client = interceptor(shortToken);
    const data = await client.post<ApiRoleResponse>(`${EXT_ROLE_BY_CC_URL}/${ccId}`);
  ...
  async uploadInsuranceImagesExt(fileInfos: FileInfo[]): Promise<UploadFilesResponse[] | null> {
    ...
    for (const fileInfo of fileInfos) {
      const { data, status } = await client.post<UploadFilesResponse>(EXT_UPLOAD_IMAGE!, fileInfo);
