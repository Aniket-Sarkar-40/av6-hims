...
import { itemStockService } from "@/services/stock/itemStock.service";
import { BaseResponse } from "@/utils/baseResponse.utils";
import { logger } from "@/utils/logger.utils";
import { Request, Response } from "express";

export const getItemStockSummary = TryCatch(async (req: Request, res: Response) => {
  logger.info("entering::getItemStockSummary::controller");
  const { ccId } = req.query as { ccId: string };
  const itemStock = await itemStockService.getAllItemStockSummary(Number(ccId));
  logger.info("exiting::getItemStockSummary::controller");
  const response = BaseResponse.success({ type: "FETCHED", data: itemStock }, "Item Stock Summary");
...
export const getItemStock = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "FETCHED", data: itemStock }, "Item Stock");
  return res.status(200).json(response);
});

export const exportItemStockExcel = TryCatch(async (req: Request, res: Response) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename=item-stock-export-${ccId}.xlsx`);
