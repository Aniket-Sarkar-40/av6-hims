...
import { currencyService } from "@/services/master/currency.service";
...
export const createCurrency = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "CREATED", data: currency }, "Currency");
...
export const getAllCurrency = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "FETCHED", data: currency }, "Currency");
...
export const getCurrencyById = TryCatch(async (req: Request, res: Response) => {
  ...
  if (!currency) {
    ...
    return res.status(404).json(BaseResponse.error({ message: "Currency Not Found" }));
  }
  logger.info("exiting::getCurrencyById::controller");
  return res.status(200).json(BaseResponse.success({ type: "FETCHED", data: currency }, "Currency"));
});
...
export const updateCurrency = TryCatch(async (req: Request, res: Response) => {
  ...
  return res.status(200).json(BaseResponse.success({ type: "UPDATED", data: updatedCurrency }, "Currency"));
});
...
export const deleteCurrency = TryCatch(async (req: Request, res: Response) => {
  ...
  return res.status(200).json(BaseResponse.success({ type: "DELETED" }, "Currency"));
});
