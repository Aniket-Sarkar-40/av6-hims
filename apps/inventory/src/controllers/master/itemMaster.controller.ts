...
export const createItemMaster = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "CREATED", data: itemMaster }, "Item Master");
  logger.info("exiting::createItemMaster::controller");
  return res.status(201).json(response);
});

export const updateItemMaster = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "UPDATED", data: updateItemMaster }, "Item Master");
  return res.status(200).json(response);
});

export const getAllItemMaster = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "FETCHED", data: itemMaster }, "Item Master");
  return res.status(200).json(response);
});

export const getItemMasterById = TryCatch(async (req: Request, res: Response) => {
  ...
  if (!itemMaster) {
    const response = BaseResponse.error({ message: generateErrorMessage("NOT_FOUND", "Item Master") });
...
export const getItemStocksByItemId = TryCatch(async (req: Request, res: Response) => {
...
export const getBulkItemSupplierPrices = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "FETCHED", data }, "Item Supplier Prices");
  return res.status(200).json(response);
});
...
export const itemExcelSampleExport = TryCatch(async (req: Request, res: Response) => {
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="sample_item.xlsx"');
