...
export const createItemStore = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "CREATED", data: itemStore }, "Item Store");
...
export const updateItemStore = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "UPDATED", data: updateItemStore }, "Item Store");
...
export const getAllItemStore = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "FETCHED", data: itemStore }, "Item Store");
  return res.status(200).json(response);
});

export const getItemStoreById = TryCatch(async (req: Request, res: Response) => {
  logger.info("entering::getItemStoreById::controller");
  const { itemStoreId } = req.query as { itemStoreId: string };

  const itemStore = await itemStoreService.getItemStoreById(Number(itemStoreId));

  if (!itemStore) {
    const response = BaseResponse.error({ message: generateErrorMessage("NOT_FOUND", "Item Store") });
