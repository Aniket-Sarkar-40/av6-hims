import { commonServiceFactory } from "@/config/core.config";
import { TryCatch } from "@/middlewares/error.middleware";
import { commonService } from "@/services/common.service";
import { shortCodeService } from "@/services/shortCode.service";
import { LockUnlockParams } from "@/types/common";
import { BaseResponse } from "@/utils/baseResponse.utils";
import ErrorHandler from "@/utils/errorHandler.utils";
import { logger } from "@/utils/logger.utils";
import { generateSuccessMessage, generateValidationErrorMessage } from "@/utils/responseMessage.utils";
import { validateUpdateDynamicShortCodeConfig } from "@/validations/service/commonService.validation";
import {
  CommonExcelRequest,
  DeleteParams,
  DropdownRequest,
...
export const fixedSearch = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = new BaseResponse({ success: true, message: "Data fetched successfully." }, searchData);
...
export const commonDropdownSearch = TryCatch(async (req: Request, res: Response) => {
...
export const fixedSearchWoPaginationController = TryCatch(async (req: Request, res: Response) => {
  ...
  const searchData = await commonServiceFactory.fixedSearchWoPaginationService({
    shortCode,
    searchText,
    searchColumns,
    sortBy,
...
export const commonExcelImport = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = new BaseResponse({ success: true, message: "Data Imported successfully." }, excelImport);
  logger.info("exiting::commonExcelImport::controller");
  return res.status(200).json(response);
});

export const commonExcelExport = TryCatch(async (req: Request, res: Response) => {
  ...
  res.setHeader("Content-Disposition", `attachment; filename="${shortCodeData.tableName}.xlsx"`);
...
export const commonUpdateStatus = TryCatch(async (req: Request, res: Response) => {
...
export const commonCreate = TryCatch(async (req: Request, res: Response) => {
  ...
  if (!shortCode) {
    throw new ErrorHandler(400, generateValidationErrorMessage("REQUIRED", `short code`));
  }
  ...
  const response = BaseResponse.success({ type: "CREATED", data }, shortCode.replace("_", " "));
...
export const commonUpdate = TryCatch(async (req: Request, res: Response) => {
  ...
  if (!pkField || !pkLocation) {
    throw new ErrorHandler(500, `Primary key configuration not found for short code: ${shortCodeData.shortCode}`);
  }

  const pkValue =
    pkLocation === "body" ? input[pkField] : pkLocation === "params" ? req.params[pkField] : req.query[pkField];
  ...
  const response = BaseResponse.success({ type: "UPDATED", data }, shortCode.replace("_", " "));

  logger.info("exiting::commonUpdate::controller");
  return res.status(200).json(response);
});

export const commonMultiCreateUpdate = TryCatch(async (req: Request, res: Response) => {
...
export const commonFSExcelExport = TryCatch(async (req: Request, res: Response) => {
  logger.info("entering::commonExcelExport::controller");

  const inp = req.body as CommonExcelRequest<DynamicShortCode>;
  const shortCodeData = await shortCodeService.getShortCodeByCode(inp.shortCode);
  ...
  res.setHeader("Content-Type", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
  res.setHeader("Content-Disposition", `attachment; filename="${inp.sheetName}.xlsx"`);
  await wb.xlsx.write(res);
  res.end();
});

export const updateConfigByCode = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success({ type: "UPDATED", data }, "Short Code Config");

  logger.info("exiting::updateConfigByCode::controller");
  return res.status(200).json(response);
});

export const getConfigByShortCode = TryCatch(async (req: Request, res: Response) => {
  ...
  const cfg = shortCodeData.config as unknown as DynamicCrudConfig;
  if (!cfg) {
    throw new ErrorHandler(404, `Configuration not found for short code: ${shortCodeData.shortCode}`);
  }
...
export const commonLockUnlock = TryCatch(async (req: Request, res: Response) => {
  ...
  const response = BaseResponse.success(
    { type: "UPDATED", data: { shortCode, id, isLock: lockUnlockResult.isLock } },
    action
  );
