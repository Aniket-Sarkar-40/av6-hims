import { getItemStoreByItemStoreNameFromDb } from "@/repository/master/itemStore.repository";
import { itemStoreService } from "@/services/master/itemStore.service";
import { ItemStoreReq, ItemStoreUpdate } from "@/types/master/itemStore";
...
export const validateIdItemStore = async (itemStoreId: number) => {
  ...
  if (!itemStore) {
    throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "Item Store"));
  }
...
export const updateIdItemStoreServiceValidation = async (input: ItemStoreUpdate): Promise<void> => {
  ...
  const itemStoreByName = await getItemStoreByItemStoreNameFromDb(input.itemStoreName);
  if (itemStoreByName && itemStoreByName.id !== input.id) {
    throw new ErrorHandler(400, generateErrorMessage("DUPLICATE_ITEM", "Item Store Name"));
  }
...
export const createItemStoreServiceValidation = async (body: ItemStoreReq): Promise<void> => {
