import { getTaxDetailsByNameFromDb } from "@/repository/master/taxDetails.repository.js";
...
export const updateIdTaxDetailsServiceValidation = async (
  body: CreateOrUpdateTaxDetails,
) => {
  logger.info(
    "entering::updateIdTaxDetailsServiceValidation::service::validation",
  );
  if (body.id) {
  ...
  if (taxDetailsByName && taxDetailsByName.id !== body.id) {
    throw new ErrorHandler(
      400,
      generateErrorMessage("DUPLICATE_ITEM", "Tax Details Name"),
    );
  }
  logger.info(
    "exiting::updateIdTaxDetailsServiceValidation::service::validation",
  );
  return;
};

export const createTaxDetailsServiceValidation = async (
  body: CreateOrUpdateTaxDetails,
) => {
  logger.info(
    "entering::createTaxDetailsServiceValidation::service::validation",
  );
  const taxDetailsByName = await getTaxDetailsByNameFromDb(body.name);
  if (taxDetailsByName) {
    throw new ErrorHandler(
  ...
  logger.info(
    "exiting::createTaxDetailsServiceValidation::service::validation",
  );
  return;
};
