...
import { getGrnByIdFromDb } from "@/repository/grn/grn.repository";
import { getCountGrnReturnDetailsFromDb, getGrnReturnByIdFromDb } from "@/repository/grn/grnReturn.repository";
...
import { CreateGrnReturnInput, GrnReturnDetailInput } from "@/types/grn/grnReturn";
...
import { CalculationMethod, RETURN_STS } from "@prisma/client";
...
export const validateIdGrnReturn = async (id: number) => {
  ...
  if (!grnReturn) {
    throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "Good Receive Note Return"));
  }
...
export const validateGrnReturnCommon = async (body: CreateGrnReturnInput): Promise<void> => {
  logger.info("entering::validateGrnReturnCommon::service::validation");

  const supplier = await itemSupplierService.getItemSupplierById(body.supplierId, true);
  if (!supplier) {
  ...
  if (grn.poNumber !== body.poNumber) {
    throw new ErrorHandler(400, generateErrorMessage("MISMATCH", "Purchase Number", "Sending purchase number"));
  }

  if (grn.poId !== body.poId) {
    throw new ErrorHandler(400, generateErrorMessage("MISMATCH", "Purchase Id", "Sending purchase id"));
  }

  if (grn.grnNumber !== body.grnNumber) {
    throw new ErrorHandler(400, generateErrorMessage("MISMATCH", "Grn Number", "Sending grn number"));
  }
  ...
  const itemIds: Set<number> = new Set(body.goodReceiveReturnDetails.map((d: GrnReturnDetailInput) => d.itemId));
  for (const itemId of itemIds) {
    const itm = await itemMasterService.getItemMasterByIdWoDto(itemId, true);
    if (!itm) {
      throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", `Item ${itemId}`));
    }
    if (itm.isReturnable === false) {
      throw new ErrorHandler(400, generateErrorMessage("INVALID_VALUE", `Item ${itemId} is not returnable`));
    }
  }
  const grnDetailsItemIds = new Set(grn.goodReceiveDetails.map((d) => d.itemId));

  const invalidGrnIds = Array.from(itemIds).filter((id) => !grnDetailsItemIds.has(id));
  ...
  for (const detail of body.goodReceiveReturnDetails) {
    if (!body.isApproval) {
      const grnDetail = grn.goodReceiveDetails.find((grnItem) => grnItem.id === detail.grnDetailId);

      if (!grnDetail) {
        throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "Good Receive Note Details"));
      }

      const itemLabel = (grnDetail as unknown as { item?: { item?: string } })?.item?.item ?? `ID ${detail.itemId}`;
      ...
      if (detail.quantity !== undefined && detail.quantity > Math.min(finalQuantity, inHandQty)) {
        throw new ErrorHandler(
          400,
          generateErrorMessage(
            "INVALID_VALUE",
      ...
      if (detail.quantity === undefined) {
        throw new ErrorHandler(
          400,
          generateErrorMessage("INVALID_VALUE", `Item ${itemLabel}: Quantity is required for amount calculation`)
        );
      }
...
export const createGrnReturnServiceValidation = async (body: CreateGrnReturnInput) => {
  logger.info("entering::createGrnReturnServiceValidation::service::validation");
...
export const updateGrnReturnServiceValidation = async (body: CreateGrnReturnInput) => {
  logger.info("entering::updateGrnReturnServiceValidation::service::validation");

  if (body.id == null) {
    logger.error("missing grnReturn id in update request");
    throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "Good Receive Note Return id"));
  }
  ...
  // check if any item is not in stock transfer details
  const notInStockTransferDetails = updatedIds.filter((id) => !existingIds.includes(id));
  if (notInStockTransferDetails.length > 0) {
    throw new ErrorHandler(
      400,
      generateErrorMessage("INVALID_FIELD", `Id ${notInStockTransferDetails.join(", ")} of Stock Transfer Details`)
    );
  }
...
export const approveGrnReturnServiceValidation = async (body: CreateGrnReturnInput) => {
  logger.info("entering::approveGrnReturnServiceValidation::service::validation");
  ...
  if (ccSettingsId) {
    ...
    if (grnReturn.ccId !== body.ccId && warehouse?.isMain === false) {
      throw new ErrorHandler(404, generateErrorMessage("MISMATCH", "Warehouse Id", "CC Id"));
    }
  } else {
    branch = await branchService.getBranchById(body.ccId, true);
    if (grnReturn.ccId !== body.ccId && branch?.isMain === false) {
      throw new ErrorHandler(404, generateErrorMessage("MISMATCH", "Branch Id", "CC Id"));
    }
  }
  ...
  const detailIds = Array.from(
    new Set(body.goodReceiveReturnDetails.map((d) => d.id).filter((id): id is number => id != null))
  );

  if (detailIds.length > 0) {
    const count = await getCountGrnReturnDetailsFromDb(detailIds, body.id);
    if (count !== detailIds.length) {
      throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "Good Receive Note Return details"));
    }
  }

  body.isApproval = true;
  if (grnReturn.status !== "PENDING") {
    throw new ErrorHandler(400, generateErrorMessage("INVALID_STATUS", "Good Receive Note Return"));
  }

  await validateGrnReturnCommon(body);

  logger.info("exiting::approveGrnReturnServiceValidation::service::validation");
};

export const rejectGrnReturnServiceValidation = async (body: { id: number; grnId: number; ccId?: number }) => {
  logger.info("entering::rejectGrnReturnServiceValidation::service::validation");
  ...
  if (grnReturn.grnId !== body.grnId) {
    throw new ErrorHandler(
      404,
      generateErrorMessage("MISMATCH", "Good Receive Note Id", "Sending Good Receive Note Id")
    );
  }
...
export const deleteGrnReturnServiceValidation = async (id: number) => {
  ...
  if (grnReturn.status !== RETURN_STS.PENDING) {
    logger.error(`Cannot delete Good Receive Note Return with id=${id} in status=${grnReturn.status}`);
    throw new ErrorHandler(400, generateErrorMessage("INVALID_STATUS", "Good Receive Note Return"));
  }
