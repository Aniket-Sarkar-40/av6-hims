...
export const storeRequisitionService = {
  ...
  async buildExcelJSWorkbookForStoreReqByFilter(
  ...
  async getStoreRequisitionPdfResponseById(id: number) {
    logger.info(
      "entering::getStoreRequisitionPdfResponseById::service id=" + id,
    );
    ...
    if (
      !(
        (storeReq.storeReqStatus === STORE_REQ_STATUS.Approved ||
          storeReq.storeReqStatus === STORE_REQ_STATUS.Partially_Approved) &&
        storeReq.storeReqAckStatus === STORE_REQ_ACK_STATUS.ACK_PENDING
      )
    ) {
      throw new ErrorHandler(
        400,
    ...
    const batchWiseDto = await toStoreRequisitionBatchWiseDTO(storeReq);
    const pdfDto = await toStoreRequisitionPdfDTO(batchWiseDto);
    logger.info("exiting::getStoreRequisitionBatchWiseById::service id=" + id);
    return pdfDto;
  },
};
