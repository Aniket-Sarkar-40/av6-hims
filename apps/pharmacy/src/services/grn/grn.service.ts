...
export const grnService = {
  ...
  async buildExcelJSWorkbookForGrnByFilter(
    ...
    if (!data.length)
      throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "GRN"));

    const dtoList: GrnDTO[] = await Promise.all(
      data.map((grn) => toGrnDTO(grn)),
    );
    ...
    for (const grn of dtoList) {
      ...
      const dueDate = grn.dueDate
        ? dayjs(grn.dueDate).format("YYYY-MM-DD")
        : "N/A";
      const netTax = grn.netTax ?? 0;
      const tax = grn.tax ?? 0;
      const paidAmount = grn.paidAmount ?? 0;
      const billDate = grn.billDate
        ? dayjs(grn.billDate).format("YYYY-MM-DD")
        : "N/A";
