...
export const grnReturnService = {
  ...
  async buildExcelJSWorkbookForGrnReturnByFilter(
    ...
    if (!data.length)
      throw new ErrorHandler(
        404,
        generateErrorMessage("NOT_FOUND", "GRN Return"),
      );

    const dtoList: GoodReceiveReturnDTO[] = await Promise.all(
      data.map((grnReturn) => toGrnReturnDTO(grnReturn)),
    );
    ...
    for (const grnReturn of dtoList) {
      ...
      [1, 3, 5].forEach(
        (i) =>
          (info1.getCell(i).font = { bold: true, color: { argb: "666161" } }),
      );
      rowIndex++;
      ...
      [1, 3, 5, 7].forEach(
        (i) =>
          (info2.getCell(i).font = { bold: true, color: { argb: "666161" } }),
      );
      rowIndex++;
      ...
      [1, 3, 5, 7, 9].forEach(
        (i) =>
          (info3.getCell(i).font = { bold: true, color: { argb: "666161" } }),
      );
      rowIndex++;

      const info4 = ws.addRow([
        "Due Date",
        dueDate,
        "Net Tax",
        netTax,
      ...
      [1, 3, 5, 7].forEach(
        (i) =>
          (info4.getCell(i).font = { bold: true, color: { argb: "666161" } }),
      );
      rowIndex++;
      ...
      [1, 3, 5, 7, 9].forEach(
        (i) =>
          (info5.getCell(i).font = { bold: true, color: { argb: "666161" } }),
      );
      rowIndex++;
