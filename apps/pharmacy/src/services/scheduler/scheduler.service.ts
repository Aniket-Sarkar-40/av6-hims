...
export async function lowStockAlert(inp: {
  ...
  const lowStockItems = await fetchLowStockItems(inp.runDate ?? new Date());
  if (lowStockItems.length === 0) {
    logger.error("No low stock items found.");
    return false;
  }
  ...
  const autoAlertAudit = await createAutoAlertAuditInDb({
    ...
    alertType: ALERT_TYPE.LOW_STOCK,
  ...
  try {
    ...
    emailTemplate.subject = `${emailTemplate.subject} ${inp.runDate?.toDateString() ?? new Date().toDateString()}`;
    ...
    const ws = wb.addWorksheet("Low Stock Items");
...
export async function expiredItemAlert(inp: {
  alertMode?: ALERT_MODE;
  runDate?: Date;
  isResend: boolean;
  resendMasterId?: number;
  ...
  const autoAlertAudit = await createAutoAlertAuditInDb({
    runDate: inp.runDate ?? new Date(),
    deliveryMethod: ALERT_DELIVERY_METHOD.EMAIL,
    alertType: ALERT_TYPE.EXPIRED_ITEMS,
    isResend: inp.isResend,
    resendMasterId: inp.resendMasterId,
    alertMode: inp.alertMode ?? ALERT_MODE.MANUAL,
  });
...
export async function expiringItemAlert(inp: {
  alertMode?: ALERT_MODE;
  runDate?: Date;
  isResend: boolean;
  resendMasterId?: number;
  ...
  const autoAlertAudit = await createAutoAlertAuditInDb({
    runDate: inp.runDate ?? new Date(),
    deliveryMethod: ALERT_DELIVERY_METHOD.EMAIL,
    alertType: ALERT_TYPE.EXPIRING_ITEMS,
    isResend: inp.isResend,
    resendMasterId: inp.resendMasterId,
    alertMode: inp.alertMode ?? ALERT_MODE.MANUAL,
  });
...
cron.schedule("05 00 * * *", async () => {
  await lowStockAlert({ alertMode: ALERT_MODE.SYSTEM, isResend: false });
});

/*==========================Expired Item Alert============================*/
cron.schedule("10 00 * * *", async () => {
  await expiredItemAlert({ alertMode: ALERT_MODE.SYSTEM, isResend: false });
});

/*==========================Expiring Item Alert============================*/
cron.schedule("15 00 * * *", async () => {
  await expiringItemAlert({ alertMode: ALERT_MODE.SYSTEM, isResend: false });
});
