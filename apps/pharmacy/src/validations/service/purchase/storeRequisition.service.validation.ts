...
export const validateStoreRequisitionCommon = async (
  ...
  if (
    body.storeReqStatus &&
    body.storeReqStatus !== "Pending" &&
    body.storeReqStatus !== "Draft"
  ) {
    throw new ErrorHandler(
      400,
  ...
  const rawItemCategoryIds: number[] = body.storeRequisitionDetails
    .map((d: StoreRequisitionDetailInput) => d.itemCategoryId)
    .filter((id): id is number => id != null);

  const itemCategoryIds = Array.from(new Set(rawItemCategoryIds));
  const [items, categoriesCount] = await Promise.all([
    getCountItemsFromDb(itemIds),
    getCountMedCategoriesFromDb(itemCategoryIds),
  ]);
  ...
  if (categoriesCount.length !== categoriesCount.length) {
    throw new ErrorHandler(
      404,
      generateErrorMessage("NOT_FOUND", "Medicine Category Size"),
    );
  }
...
export const updateStoreRequisitionServiceValidation = async (
  ...
  if (
    currReq.storeReqStatus === "Approved" ||
    currReq.storeReqStatus === "Partially_Approved"
  ) {
    throw new ErrorHandler(
      404,
      generateErrorMessage("INVALID_STATUS", "Store requisition"),
...
export const rejectStoreRequisitionServiceValidation = async (
  ...
  if (currStoreReq.warehouseId !== body.ccId) {
    throw new ErrorHandler(403, generateErrorMessage("ACCESS_FAIL"));
  }
  if (
    currStoreReq.storeReqStatus !== "Pending" &&
    currStoreReq.storeReqStatus !== "Draft"
  ) {
    throw new ErrorHandler(
      400,
      generateErrorMessage("INVALID_STATUS", "Store requisition"),
...
export const approveStoreRequisitionServiceValidation = async (
  ...
  if (
    currStoreReq.storeReqStatus !== "Pending" &&
    currStoreReq.storeReqStatus !== "Partially_Approved"
  ) {
    throw new ErrorHandler(
      400,
      generateErrorMessage("INVALID_STATUS", "Store requisition"),
  ...
  if (
    currStoreReq.storeReqAckStatus === "ACK_RECEIVED" &&
...
export const acknowledgeStoreRequisitionServiceValidation = async (
  ...
  if (
    currStoreReq.storeReqStatus !== "Approved" &&
    currStoreReq.storeReqStatus !== "Partially_Approved"
  ) {
    throw new ErrorHandler(
      400,
...
export const deleteStoreRequisitionServiceValidation = async (id: number) => {
  ...
  if (
    srn.storeReqStatus !== STORE_REQ_STATUS.Draft &&
    srn.storeReqStatus !== STORE_REQ_STATUS.Pending
  ) {
    throw new ErrorHandler(
      400,
      generateErrorMessage("INVALID_STATUS", "Store requisition"),
