import { getPattern, ToggleActive } from "av6-core";
import { NextFunction, Request, Response } from "express";
import Joi from "joi";
import { BaseResponse } from "@repo/shared/utils/baseResponse.utils.js";
import { SHORT_CODE } from "@repo/shared/utils/shortCode/pharmacy.shortCode.utils.js";

export const commonFilterRequestSchema = Joi.object({
  pageNo: Joi.number().integer().min(1).messages({
  ...
  sortDir: Joi.string().valid("ASC", "DESC").messages({
    "any.only": "sortDir must be either ASC or DESC",
    "string.base": "sortDir must be a string",
  }),
  searchText: Joi.string().messages({
    "string.base": "searchText must be a string",
  }),
});

// Schema for CommonFilterWithDate (extends CommonFilterRequest)
export const commonFilterWithDateSchema = commonFilterRequestSchema.keys({
...
export const validateApprovalRequest = (
  ...
  const { error } = commonApproveSchema.validate(req.body, {
    abortEarly: false,
  });
...
export const validateStartFlowRequest = (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  const { error } = startFlowReqSchema.validate(req.body, {
    abortEarly: false,
  });
...
export const validateCommonCreate = (
  ...
  const { error } = commonCreateSchema.validate(req.body, {
    abortEarly: false,
  });
