...
const runtimeDataModel = (db as any)._runtimeDataModel;
...
const getModelMeta = (delegateKey: string): PrismaModelMeta => {
...
export const getModelScalarFieldNames = (delegateKey: string): string[] => {
...
export const assertModelScalarField = (
  ...
  const field = getModelMeta(delegateKey).fields.find(
    (item) =>
      item.name === fieldName &&
