import { markCronFailed, markCronInProgress, markCronSuccess, updateCronMessageById, } from "@/repository/cron/cronDetails.repository.js";
...
export const cronDetailsService = {
    async start(input) {
        ...
        await updateCronMessageById(ctx.id, withAttemptPrefix(validation.attempt, CRON_MAX_RETRIES, "in_progress"));
...
export const runCronTask = async (options) => {
    ...
    try {
        ...
        await cronDetailsService.success({
            ctx,
            message: withAttemptPrefix(attempt, CRON_MAX_RETRIES, buildMessage?.(result)),
        });
        ...
        await cronDetailsService.fail({
            ctx,
            err: new Error(withAttemptPrefix(attempt, CRON_MAX_RETRIES, err instanceof Error ? err.message : String(err))),
        });
