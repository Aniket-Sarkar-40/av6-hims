import { getCronDetailsById, getLatestCronByRunKey, markCronFailedById, } from "@/repository/cron/cronDetails.repository.js";
...
export const validateIdCronDetails = async (id) => {
    logger.info("entering::validateIdCronDetails::serviceValidation");
    const cron = await getCronDetailsById(id);
    if (!cron) {
        throw new ErrorHandler(404, generateErrorMessage("NOT_FOUND", "Cron Details"));
    }
    logger.info("exiting::validateIdCronDetails::serviceValidation");
    return cron;
};
export const beginCronDetailsServiceValidation = async (input) => {
    ...
    if (latest.status === CronStatus.IN_PROGRESS) {
        ...
        await markCronFailedById(latest.id, `stale in-progress takeover (ageMs=${ageMs})`);
    }
